...
export default class GioHang extends Component {
  renderCard = () => {
    return this.props.card.map((element) => {
      return (
        <tr key={element.id}>
          <td>{element.id}</td>
          <td>
          ...
          <td>
            {(element.price * element.soLuong).toLocaleString("it-IT", {
              style: "currency",
              currency: "VND",
            })}
          </td>
          <td>
            <button
              onClick={() => this.props.deletePhone(element)}
      ...
      <div>
        <div className="d-flex justify-content-end mb-2">
          <button
            ...
            data-target="#gioHang"
          >
            GIỎ HÀNG ({this.props.card.length})
          </button>
        </div>
        <div
